...
public class LocationActivity extends FragmentActivity {
	...
	private void addGeoFence(String key, LatLng point) {
		...
		circleOptions.center(point).radius(GEOFENCE_RADIUS)
				.strokeColor(android.graphics.Color.BLUE).strokeWidth(2);
	...
	private void codeForThePresentation() {
		...
		LocationClient mLocationClient = new LocationClient(this,
				connectionCallback, connectionFailedListener);
		mLocationClient.connect();
	...
	private class LocationCallback implements ConnectionCallbacks,
		...
		private void handleLocation(Location location) {
			...
			mLocationStatus.setText("Location changed @"
	...
	private class ActivityRecognitionCallback implements ConnectionCallbacks,
		...
		public void onConnected(Bundle connectionHint) {
			...
			mActivityRecognitionPendingIntent = PendingIntent.getService(
					LocationActivity.this, 0, intent,
		...
		public void onDisconnected() {
			Log.v(LocationActivity.TAG,
					"Activity Recognition Client Disconnected");
		}

		@Override
		public void onConnectionFailed(ConnectionResult result) {
			Log.v(LocationActivity.TAG,
					"Activity Recognition Client connection failed "
							+ result.getErrorCode());
		}
	};
	...
	private class GeoFenceCallback implements OnMapLongClickListener,
		...
		public void onMapLongClick(LatLng point) {
			Log.v(LocationActivity.TAG, "onMapLongClick == " + point.latitude
					+ "," + point.longitude);
		...
		private void addGeoFences() {
			...
			for (Map.Entry<String, Circle> entry : mGeoFences.entrySet()) {
				...
				Log.v(LocationActivity.TAG,
						"points == " + circle.getCenter().latitude + ","
		...
		private void removeGeoFences() {
			...
			for (Map.Entry<String, Circle> entry : mGeoFences.entrySet()) {
				...
				if (circle != null) {
					circle.remove();
					id--;
					Log.v(LocationActivity.TAG, "RemoveGeoFence requestId == "
							+ requestId);
		...
		public void onRemoveGeofencesByRequestIdsResult(int statusCode,
			...
			Log.v(LocationActivity.TAG, "Removed Geofence "
					+ statusCodeToString(statusCode) + " " + builder.toString());
			mGeoFenceStatus
					.setText("Removed Geofences request_ids = "
							+ builder.toString() + " "
							+ statusCodeToString(statusCode));
		}
	...
	protected void onNewIntent(Intent intent) {
		...
		boolean receiverStarted = intent.getBooleanExtra("RECEIVER_STARTED",
				false);
